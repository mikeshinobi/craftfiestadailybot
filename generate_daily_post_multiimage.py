...
You must respond with ONLY a JSON object (no markdown fences, no commentary) with exactly these keys:
  ...
  "html_body": the full post body as simple HTML (use <p>, <h2>, <ul> as needed, no <html>/<body> tags).
               Insert the exact placeholder token [[IMAGE_1]] at the spot where the main/featured
               image should appear (usually near the top). If you decide to use more images, insert
               [[IMAGE_2]], [[IMAGE_3]], etc. at the spots in the body where each should appear.
  "images": a list of 1 to {MAX_IMAGES} objects, each with:
               "prompt": a description for an AI image generator to create a REALISTIC,
                         ...
                         illustration, cartoon, or digital art) that would make sense at that
                         point in the post. Be concrete about the real-world scene, subject, and
                         setting -- avoid generic or abstract imagery. Explicitly include words
                         like "photograph" or "photorealistic" in the prompt itself. No text or
                         words should appear in the image. If the topic involves skin, needles,
                         medical procedures, or bodily trauma (e.g. tattoos, piercings, injuries,
                         skin conditions), keep the image clean and professional -- e.g. a
                         studio/clinic setting, tools laid out, a finished/healed result, or a
                         product shot -- rather than depicting wounds, blood, or damaged skin,
                         since realistic depictions of those can get incorrectly flagged as
                         harmful content by the image generator.
               "alt": short accessibility alt text describing the image
             The first item corresponds to [[IMAGE_1]] (the main/featured image), the second to
             [[IMAGE_2]], and so on. Only use more than one image if it genuinely improves the
             post -- most posts should still just use one; reserve extra images for cases where
             a post covers multiple distinct steps, examples, or sub-topics that would each
             benefit from their own picture. Don't add images just to hit a higher count.
